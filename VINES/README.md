...
3.1.1 Open three terminals, launch the vines_estimator , rviz and play the bag file respectively. Take MH_01 for example
```
    roslaunch vines_estimator euroc.launch 
...
```
    roslaunch vines_estimator euroc_no_extrinsic_param.launch
```
...
Set the **pose_graph_save_path** in the config file (YOUR_VINS_FOLEDER/config/euroc/euroc_config.yaml). After playing MH_01 bag, input **s** in vines_estimator terminal, then **enter**. The current pose graph will be saved. 
